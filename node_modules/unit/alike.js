 ...
 * unlike like, alike considers arrays equivalent if
 * they have the same members - regardless of the order.
 */
...
function say(){
    if (false){
        var ar = arguments;
        var ar_array = _.toArray(ar);
        console.log.apply(console, ar_array);
    }
}

function _like_array(a, b) {
 say('_like_array: comparing', a, 'to', b);

    if (a.length != b.length) {
      say('array lengths are different: ', a, b);
        return false;
    }
    var like = true;

    a = _.sortBy(a.slice(0), _pipe);
    b = _.sortBy(b.slice(0), _pipe);

    a.forEach(function(value, index) {
        if (like) {
          say('comparing ', value, 'to', b[index]);
    ...
    if (like) {
     ...
     //   console.log('sad buddha!');
    }
    return like;
}

function _pipe(x){return x}
...
function _like_object(a, b) {
    ...
    if (a_keys.length != b_keys.length) {
        say('different count of keys', a_keys, b_keys);
        return false;
    }
   ...
   //     console.log(a_keys.join(''), '!=', b_keys.join(''));
    ...
    a_keys.forEach(function(key) {
        if (like) {
            ...
            if (!like) {
               say('different values for', key, ': ', a[key], b[key]);
            }
        }
    });

   say('smilin jesus!', a, b);
    return like;
}

function _like(a, b) {
    say ('alike comparing', a, b);
    if (_.isUndefined(a)) {
        if (!_.isUndefined(b)) {
            return false;
        }
        ...
        if (!_.isArray(b)) {
            say('comparing array ', a, 'to non-array', b);
            return false;

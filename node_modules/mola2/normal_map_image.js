...
module.exports = function(data, x, y, zoom) {
    var h = data[y][x];
    if (h < 0){
        var out = [125, 125, 255, 255];

        for (var i = 0; i < 2; ++i){
            out[i] += Math.round(Math.random() * 5);
        }
        return out;
    }
    //var lat = _degree_lat(y, data);
    // var cos = _cos_degree(lat);
    // note - should be 500 / zoom - exaggerating for effect
    var lat_m_per_point = 50 / zoom; // the higher/lower you go, the smaller the lateral distance between measurements.
    // * Math.max(0.001, Math.abs(cos)
    var lon_m_per_point = 50 / zoom;
    var lat_traverse = 0;
    var lon_traverse = 0;
    ...
    switch (q[0]) {
        ...
        case 'left':
            ...
            lon_traverse = (right - left) / (-4 * lon_m_per_point);
    ...
    switch (q[1]) {
        case "top":
            ...
            lat_traverse = (top - bottom) / (4 * lat_m_per_point);

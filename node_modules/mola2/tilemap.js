...
module.exports = function(tile) {
    if (tile.hasOwnProperty('image_data')) {
        this.manifest = tile.image_data.manifest;
    } else {
    ...
    this.parse_manifest();
    this.data = tile.heights;
    this.rows = this.cols = tile.scale;
    ...
    calculate_normal: function(i, j) {
        ...
        if (i == 0) {
            ...
            if (j == 0) {
                ...
                x = mu.clamp(left - right + 127, 0, 255);
                y = mu.clamp(top - bottom + 127, 0, 255);
    
                var radius = Math.sqrt((x * x) + (y * y));
                if (radius > 255){
                    var r_scale = radius / 255;
                    x /= r_scale;
                    y /= r_scale;
        ...
        return [parseInt(x), parseInt(y), 255, 255];
    },

    parse_manifest: function() {
        console.log('parsing manifest ', this.manifest.notes);
        this.rows = parseInt(this.manifest.lines);
        this.cols = parseInt(this.manifest.line_samples);
    }
